...
const Coin = () => {
  ...
  // Fetch coin data once
  useEffect(() => {
    fetchCoinData();
  }, [currency]);

  // Fetch historical data whenever days or currency changes
  useEffect(() => {
    fetchHistoricalData();
  }, [currency, days]);
      ...
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-6xl p-8">
        <div className="flex flex-col md:flex-row items-start gap-8">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left w-full md:w-1/3">
            ...
            <p className="text-gray-600 uppercase tracking-wide text-base">{coinData.symbol}</p>
            ...
            <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-gray-700 w-full">
              ...
              <div className="bg-gray-100 rounded-lg p-4">
                ...
                <p className="text-lg font-bold text-gray-800">
                  {currency.symbol}
                  {formatNumber(coinData.market_data.total_volume[currency.name])}
                </p>
              </div>

              <div className="bg-gray-100 rounded-lg p-4">
                <p className="font-medium">Market Cap Rank</p>
                <p className="text-lg font-bold text-gray-800">#{coinData.market_cap_rank}</p>
              </div>
